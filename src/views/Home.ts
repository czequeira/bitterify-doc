...
const HeroSection = section([
  div().setClasses('w-full h-full top-0 left-0 absolute bg-b1 opacity-50'),
  div([
    ...
    div([
      a('Getting started', '#').setClasses(
        'w-full text-center px-4 py-2 m-1 rounded bg-b4 hover:bg-b5 hover:text-gray-700 shadow',
      ),
      a('Github', '#').setClasses(
        'w-full text-center px-4 py-2 m-1 rounded bg-b2 hover:bg-b3 shadow',
      ),
    ]).setClasses(
