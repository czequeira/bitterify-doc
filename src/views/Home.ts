...
const HeroSection = section([
  ...
  div([
    ...
    p(['mas texto aqui y un poco mas largo con mejor explicacion']).setClasses(
      'text-lg sm:text-xl md:text-2xl',
    ),
    div([
      a('Getting started', '#').setClasses(
        'w-full text-center px-4 py-2 m-2 rounded bg-red-500 hover:bg-red-600 shadow',
      ),
      a('Github', '#').setClasses(
        'w-full text-center px-4 py-2 m-2 rounded bg-gray-500 hover:bg-gray-600 shadow',
      ),
    ]).setClasses(
      'flex flex-col w-full p-4 sm:block text-center sm:text-lg md:text-2xl',
    ),
  ]).setClasses(
    'h-full w-full px-8 md:px-24 text-center flex flex-col absolute top-0 left-0 justify-center items-center',
  ),
  img('images/pexels-tom-cordner-1399097.jpg').setClasses(
